...
function processSum(numberList, callback) {
  const numberTotal = numberList.reduce((sum, currentNum) =>  sum + currentNum, 0);
...
function processDuplicateFree(list, callback) {
  ...
  const duplicateFree = [];
  hasNum.forEach((key) => duplicateFree.push(key));
...
function getFullNames(runners) {
  const runnerNames = runners.map(runner => `${runner.last_name}, ${runner.first_name}`);
  return runnerNames;
}
...
function firstNamesAllCaps(runners) {
  const runnerFirstNamesCAPS = runners.map(runner => `${runner.first_name.toUpperCase()}`);
  return runnerFirstNamesCAPS;
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  const filteredRunnersBySize = runners.filter((runner) => runner.shirt_size === tShirtSize);
  return filteredRunnersBySize;
}
...
function tallyUpDonations(runners) {
  const totalDonations = runners.reduce((sum, runner) => sum + runner.donation, 0);
